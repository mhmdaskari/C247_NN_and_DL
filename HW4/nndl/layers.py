...
def relu_forward(x):
    ...
    out = x.copy()
    out[out<=0] = 0
    # another method: out = np.where(x<=0, 0, x)
...
def relu_backward(dout, cache):
    ...
    dx = dout.copy()
    dx[x<=0] = 0
    # another method: dx = np.where(x<=0, 0, dout)  
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        normalized_x = (x - minibatch_mean)/np.sqrt(minibatch_var + eps)
        out = gamma * normalized_x + beta
        
        cache = (x, gamma, eps, minibatch_mean, minibatch_var, normalized_x)
...
def batchnorm_backward(dout, cache):
    ...
    x, gamma, eps, minibatch_mean, minibatch_var, normalized_x = cache
    N = x.shape[0]

    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout*normalized_x, axis=0) #Element-wise multiplication of dout and normalized_x
    dx_hat = dout*gamma #dim: (N,D)
    da = 1/np.sqrt(minibatch_var+eps)*dx_hat #dim: (N,D)
    dmu = -1/np.sqrt(minibatch_var+eps)*np.sum(dx_hat, axis=0)
    de = -1/2/(minibatch_var+eps)**(3/2)*(x-minibatch_mean)*dx_hat
    dvar = np.sum(de, axis=0)
    dx = da + 2*(x-minibatch_mean)/N*dvar + 1/N*dmu
