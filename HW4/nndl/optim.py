...
def rmsprop(w, dw, config=None):
    ...
    """
    if config is None: config = {}
    config.setdefault('learning_rate', 1e-2) # Epsilon
    config.setdefault('decay_rate', 0.99) # Beta
    config.setdefault('epsilon', 1e-8) # Nu
...
def adam(w, dw, config=None):
    ...
    """
    if config is None: config = {}
    config.setdefault('learning_rate', 1e-3) #Epsilon
    ...
    v_tilda = 1/(1 - config['beta1']**config['t'])*config['v']

    config['a'] = config['beta2']*config['a'] + (1-config['beta2'])*dw**2
    a_tilda = 1/(1 - config['beta2']**config['t'])*config['a']
