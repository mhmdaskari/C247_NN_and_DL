...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dz, dw2, grads['b2'] = affine_backward(d_out, fc2_cache)
        grads['W2'] = dw2 + self.reg*W2
        da = relu_backward(dz, relu_cache)
        _, dw1, grads['b1'] = affine_backward(da, fc1_cache)
...
class FullyConnectedNet(object):
        ...
        for i in np.arange(1, self.num_layers+1):
          ...
          if i == 1:
            self.params[weight_name] = np.random.normal(
                loc=0, scale=weight_scale, size=(input_dim, hidden_dims[i-1]))
          ...
          #Output layer
            ...
            self.params[weight_name] = np.random.normal(
                loc=0, scale=weight_scale, size=(hidden_dims[i-2], num_classes))
            self.params[bias_name] = np.zeros(num_classes)

          #The rest
          else:
            self.params[weight_name] = np.random.normal(
                loc=0, scale=weight_scale, size=(hidden_dims[i-2], hidden_dims[i-1]))
            self.params[bias_name] = np.zeros(hidden_dims[i-1])
            if self.use_batchnorm:
              self.params[gamma_name] = np.ones(hidden_dims[i-1])
              self.params[beta_name] = np.zeros(hidden_dims[i-1])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers+1):
          ...
          if i == 1:
            if self.use_batchnorm:
              H_app, H_cache_app = affine_batchnorm_relu_forward(
                  X, self.params[weight_name], self.params[bias_name], self.params[gamma_name], self.params[beta_name], self.bn_params[i-1])
              
            else:
              H_app, H_cache_app = affine_relu_forward(
                  X, self.params[weight_name], self.params[bias_name])
            
            if self.use_dropout:
              H_app, dropout_cache_app = dropout_forward(H_app, self.dropout_param)
              dropout_cache.append(dropout_cache_app)
          ...
          #Output layer
            ...
            scores, H_cache_app = affine_forward(
                H[i-2], self.params[weight_name], self.params[bias_name])
            H_cache.append(H_cache_app)
          ...
          #The rest
            ...
            if self.use_batchnorm:
              H_app, H_cache_app = affine_batchnorm_relu_forward(
                  H[i-2], self.params[weight_name], self.params[bias_name], self.params[gamma_name], self.params[beta_name], self.bn_params[i-1])
        ...
        for i in range(self.num_layers, 0, -1):
          ...
          beta_name = "beta{}".format(i)
          loss += 0.5*self.reg * \
              np.sum(self.params[weight_name]**2)

          #Output layer
          if i == self.num_layers:
            dH_i, dW, db = affine_backward(dLbydZ, H_cache[i-1])
          ...
          #The rest
            ...
            if self.use_dropout:  
              dH_i = dropout_backward(dH_i, dropout_cache[i-1])

            if self.use_batchnorm:
              dH_i, dW, db, dgamma, dbeta = affine_batchnorm_relu_backward(
                  dH_i, H_cache[i-1])
              grads[weight_name] = dW + self.reg*self.params[weight_name]
              grads[bias_name] = db
              ...
              dH_i, dW, db = affine_relu_backward(dH_i, H_cache[i-1])
              grads[weight_name] = dW + self.reg*self.params[weight_name]
              grads[bias_name] = db
              dHs.append(dH_i)
