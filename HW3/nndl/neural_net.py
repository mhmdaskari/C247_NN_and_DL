...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    h1 = np.dot(X, W1.T) + b1 #dim: (N,D)*(D,H) + (H,) -> (N,H)
    h1[h1<=0]=0
    scores = np.dot(h1,W2.T) + b2 #dim: (N,H)*(H,C) + (C,) -> (N,C)
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
    ...
    numerator = np.exp(scores[range(N),y]) #dim: (N,C)
    denominator = np.sum(np.exp(scores), axis=1) #dim: (N,)
    loss = -1/N * np.sum(np.log(numerator/denominator)) + reg*0.5*(np.sum(W1**2) + np.sum(W2**2))
    ...
    probs = np.exp(scores)/np.sum(np.exp(scores), axis=1, keepdims=True) #dim: (N,C)
    probs[range(N),y] -= 1
    probs /= N
    
    #Last hidden layer derivatives
    grads['W2'] = np.dot(probs.T, h1) + reg*W2 #dim: (C,H)
    grads['b2'] = np.sum(probs.T, axis=1) #dim: (C,)
    dh1 = np.dot(probs, W2) #dim: (N,H)
    
    #Relu backpropogation
    dh1[h1<=0] = 0

    #First hidden layer derivatives
    grads['W1'] = np.dot(dh1.T, X) + reg*W1 #dim: (H,D)
    grads['b1'] = np.sum(dh1.T, axis=1) #dim: (H,)
  ...
  def predict(self, X):
    ...
    scores = self.loss(X) #y=None -> scores, dim: (N,C)
    scores = (scores.T - np.max(scores, axis=1)).T  # to control the overflow
    probs = np.exp(scores)/np.sum(np.exp(scores), axis=1, keepdims=True)
    y_pred = np.argmax(probs, axis=1) #dim: (N,)
